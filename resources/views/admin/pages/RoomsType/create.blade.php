                ...
                ">
                        <div class="card card-info">
                            <div class="card-header">
                                <h3 class="card-title"><i class="fas fa-plus mr-1"></i>Rooms Type </h3>
                            </div>
                            <!-- /.card-header -->
                            @include('admin.layouts.alerts.success')
                            @include('admin.layouts.alerts.errors')
                            <!-- form start -->
                            <form class="form-horizontal" action="{{ route('roomtype.store') }}" method="post">
                                @csrf
                                <div class="card-body">
                                    <div class="row clearfix">
                                        <div class="col-sm-6 mb-2 mt-2">
                                            <div class="form-floating input-group">
                                                ...
                                                <input type="Type" class="form-control" placeholder="Type" name="type"
                                                    id="Type" required>
                                                {{-- valid --}}
                                                    ...
                                                    @foreach ($rooms as $key => $room)
                                                        <tr class="odd">
                                                            <td class="dtr-control sorting_1" tabindex="0">
                                                                {{ $room->type }}</td>
                                                            <td>{{ $room->capacity }}</td>
                                                            <td>{{ $room->abbreviation }}</td>
                                                            <td>{{ $room->note }}</td>
                                                            <td class="d-flex">
                                                                <button type="submit" class="btn btn-success mr-2 ">
                                                                    <a href="{{ route('roomtype.edit', $room->id) }}"
                                                                    ...
                                                                    <div class="modal-dialog">
                                                                        <div class="modal-content w-50 m-auto p-2">
                                                                            ...
                                                                            <div class="modal-footer">
                                                                                <form method="post"
                                                                                    action="{{ route('roomtype.destroy', $room->id) }}">
                                                                                    @csrf
                                                                                    @method('DELETE')
                                                                                    <button type="submit"
